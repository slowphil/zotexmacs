...
;; COPYRIGHT   : (C) 2023-2026  Philippe Joyez
;;
...
(when (supports-zotexmacs?)
  ...
  (if (not (cpp-has-preference? "tls-server")) (set-preference "tls-server" "off"))

  ;(display* "zotero server on? " (get-boolean-preference "zotero-server") "\n")

  (when (and (get-boolean-preference "zotero-server") (not (get-boolean-preference "tls-server"))) ; if tls-server is set disable the plugin
...
;; define a login for connecting to texmac's server
    (server-set-user-information "zotero" "zotero-remote-cite" '((password "clear" "zotero")) "" "no")
...
;; if there are adjascent cite tags, merge them all        
  ...
  (tm-widget (plugin-preferences-widget name)
    (:require (== name "zotexmacs"))
    (dynamic (zotexmacs-preferences-widget))
  )
    ...
    (if (use-menus?)
     (-> "Zotexmacs plugin"
       ...
       (if (not (get-boolean-preference "zotero-server")) 
         ("activate plugin at next start" (toggle-preference "zotero-server")))
